#!usr/bin/env python3
"""
From: https://github.com/conductor-oss/python-sdk/blob/c99e2cf9871c21f7a64d823126ee1b77989b00ad/examples/agents/81_chat_repl.py
...
""" + "\n".join(
    f"  {name:12s}  {desc}" for name, (desc, _) in _TASK_IMPLEMENTATIONS.items()
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Chat REPL with a long-running agent via WMQ."
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        ...
        with AgentRuntime() as runtime:
            if args.resume:
                ...
                handle = runtime.start(
                    agent, "Begin. Wait for the user's first message."
                )
                execution_id = handle.execution_id
            ...
            while True:
                ...
                if user_input.lower() == "/disconnect":
                    print(
                        "Disconnected. Resume later with: python 81_chat_repl.py --resume"
                    )
                    break
